...
public class Homework1 {
    ...
    public boolean streamPipeline1() {
        return countries.stream().anyMatch(c -> c.getName().toLowerCase(Locale.ROOT).contains("island"));
    }
    ...
    public Optional<String> streamPipeline2() {
        return countries.stream()
                .filter(c -> c.getName().toLowerCase(Locale.ROOT).contains("island"))
                .map(c -> c.getName())
                .findFirst();
    }
    ...
    public void streamPipeline3() {
        countries.stream()
                .filter(c -> c.getName().toLowerCase(Locale.ROOT).charAt(0) ==
                        c.getName().toLowerCase(Locale.ROOT).charAt(c.getName().length()-1))
                .map(c -> c.getName())
    ...
    public void streamPipeline7() {
        countries.stream()
                .sorted(Comparator.comparing(c -> c.getTimezones().size()))
                .map(c -> c.getName())
                .forEach(System.out::println);
    }
...
ed by the language code "es").
    ...
    public long streamPipeline9() {
        return countries.stream()
                .filter(c -> c.getTranslations().containsKey("es") == false)
                .count();
    }
